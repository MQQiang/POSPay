...
- (void)textChanged{
    int limit = 6;
    if (self.passwordField.text.length > limit) {
        self.passwordField.text = [self.passwordField.text substringToIndex:limit];
    };
    ...
    if (changedLengthOfPassword>self.currentLength) {//增加长度
        ...
        [currentImage setImage:[UIImage imageNamed:@"box_filled"]];
    }
    else if(changedLengthOfPassword == self.currentLength){
        
    }
    else{//减少长度
        UIImageView *currentImage = self.viewArray[changedLengthOfPassword];
        [currentImage setImage:[UIImage imageNamed:@"box_empty"]];
    }
